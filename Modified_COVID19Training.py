...
def main():
    ...
    parser.add_argument('--dataset_path', type=str, default=r'D:\PythonProject\COVID19\COVID-19_Radiography_Dataset\ImageDataset', help='Dataset path')
    parser.add_argument('--sampler', action='store_false', help='Add sampler to train loader or Use weighted loss')
    parser.add_argument('--drop_class', type=str, default=['Viral_Pneumonia'], nargs='*', help='Drop class') # Drop the viral pneumonia class to balance the dataset
    parser.add_argument('--model', type=str, default='custom_inception', help='Model name')
    ...
    args = parser.parse_args(['--sampler', '--noise', '--norm'])
    ...
    net_dict = {
        ...
        'custom_inception': CustomNetwork.custom_inception,
        'baseline': CustomNetwork.baseline
    }

    if args.model == 'baseline':
        net = CustomNetwork.baseline(size=args.resize).to(device)
    else:
    ...
    fig_saving_path = f'{models_path}/loss_accuracy_{args.model}.png' # figur saving path
    ...
    # get batch loader
    train_loader, val_loader, test_loader, weights = LoadData.get_loader(dataset_path=args.dataset_path,
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=5, gamma=args.scheduler) # change lr every 5 epoches
